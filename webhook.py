...
async def add_niche_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if added:
            await update.message.reply_text(f"✅ Added niche: {niche}")
        else:
            await update.message.reply_text("⚠️ That niche already exists or is invalid.")
...
async def idea_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lower_request.startswith("give meme"):
        ...
        mode = "post"
        request_text = raw_request[len("give me post ideas"):].strip()
    elif lower_request.startswith("give post ideas"):
        mode = "post"
        request_text = raw_request[len("give post ideas"):].strip()
...
async def image_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        visual_context = await asyncio.to_thread(
            analyze_image,
            image_bytes,
            user_instruction,
        )
        ...
        if command == "research":
            ...
            intelligence = await asyncio.to_thread(
                generate_intelligence,
                research,
                "image-assisted manual research",
            )
            await status.delete()
            await send_message(update, intelligence)
            return
        ...
        if command == "create":
            create_request = (
                user_instruction or
                "Create the most useful crypto content based on what is shown in this image."
            )
        ...
        idea_request = (
            idea_request or "Create ideas from the subject and visual details in this image."
        ) + "\n\nImage context:\n" + visual_context
...
def setup_handlers():
    ...
    # Telegram CommandHandler intentionally does not process commands in media captions.
    # This handler provides the image-aware command path.
...
@app.get("/")
def health():
    return jsonify({"service": "crypto-intelligence-bot", "status": "ok"})
...
async def startup():
    ...
    if render_url:
        ...
        logger.info(
            "RENDER_EXTERNAL_URL not set. Running without webhook registration for local testing."
        )
